...
def shorten(
    ...
    calling this function with same urls does not return the same outputs.
    A hash function is used in this function instead of randomly generating a key for each url.
    The reason is that a hash function (without a salt) could be used to generate the same key for the same url
    ...
    # a couple of things are mixed together in order to create unique keys
    ...
    # very rare because of the mixture we created above.
    return ''.join(
